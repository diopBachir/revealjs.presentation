...
**{{< fa angle-right >}} L’exploitation du modèle fréquentiel** permet d’estimer la probabilité ou la fréquence d'occurrence des quantiles spécifiques de débits.]{.column-body-text}
...
[
    **{{< fa angle-right >}}** En contexte non-stationnaire, la méthode traditionnelle de l’analyse fréquentielle est inapplicable en raison du rejet des hypothèses de base (stationnarité et homogénéité).<br>
    **{{< fa angle-right >}}** Une façon de prendre en compte cette tendance est d’établir une relation linéaire entre les paramètres du modèle fréquentiel et une ou plusieurs co-variables (le temps par exemple).<br>
    ...
    **{{< fa angle-right >}}** Pour la **GEV** par exemple, exprimons le paramètre de position **$\mu$** comme une fonction linéaire du temps du temps **$\mu_{(t)}$**. Un quantile de débit au temps **$t$** peut être obtenu par la fonction quantile **$Q_t = GEV_{(μ(t)}, \delta, \xi)$**,  avec **$Q_t$** représentant le quantile estmé, **$\mu_{(t)} = \mu_0 + \mu1_{(t)}$** et **$t$** étant un indicateur de temporelle.<br><br>
...
<br>
![résultats de la comparaison des distributions basées sur les incertitudes autour des estimations de quantiles. Pour chaque bassin, la meilleure distribution est affichée avec le pourcentage d’incertitudes correspondant.](assets/images/Meilleurs_distribution_et_repartition_spatiale_des_incertitudes.jpeg)
...
Sur la base de l'évaluation des modèles fréquentiels, les paramètres de la **GEV** et de mla **Gumbel** ont été spatialisés. La **GEV** a quatre paramètres et la **Gumbel** en a trois.<br>
...
[**{{< fa angle-right >}}** Le test de tendance de **Mann-Kendall** appliqué au seuil de 05% a permis de détecter 16 stations avec des tendances significatives (dont 07 avec des tendances positives et 09 avec des tendances négatives.)<br><br>
